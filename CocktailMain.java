...
public class CocktailMain {
    ...
    public static void main(String[] args) {
        ...
        println("Lista celor " + Cocktail.nrCocktails + " cocktail-uri este:");
        for (int i = 0; i < nrCocktails; i++) {
            println(bar[i].nume + " ");
        ...
        println("Gradul mediu de alcool al tuturor bauturilor este de: " + (avgGradAlcool / totalNrIngrediente));

        double avgPriceIngredient = 0;
        for (int i = 0; i < nrCocktails; i++) {
            avgPriceIngredient += bar[i].sumPrice();
        }
        println("Pretul mediu al tuturor ingredientilor este de: " + (avgPriceIngredient / totalNrIngrediente));

        float maxPriceIngredient = 0;
        for (int i = 0; i < nrCocktails; i++)
            if (maxPriceIngredient < bar[i].maxPrice())
                maxPriceIngredient = bar[i].maxPrice();
        println("Ingredientul cel mai costisitor este: " + maxPriceIngredient);

        int maxgradAlcool = 0;
        for (int i = 0; i < nrCocktails; i++)
            if (maxgradAlcool < bar[i].maxAlcool())
                maxgradAlcool = bar[i].maxAlcool();
        println("Ingredientul cel mai mare grad alco este: " + maxgradAlcool);
